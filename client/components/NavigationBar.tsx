  ...
  useEffect(() => {
    const compute = () => {
      ...
      navigationItems.forEach((it) => {
        const el = itemRefs.current[it.label];
        if (el) w[it.label] = el.offsetWidth;
      });
      if (homeRef.current) w.__home = homeRef.current.offsetWidth;
      setWidths(w);
    };
    ...
    <div className="w-[2160px] mx-auto bg-white relative z-nav">
      ...
      <div className="flex items-center justify-between px-[65px] py-[96px]">
        ...
        <div className="nav-item active">
          <div ref={homeRef} className="w-[79px] h-[79px] flex items-center justify-center relative">
            ...
            <div
              ...
              style={{ width: widths.__home ? `${widths.__home}px` : undefined }}
            />
          </div>
        </div>
          ...
          {navigationItems.map((item, index) => (
            <div key={item.label} className={`flex items-center nav-item ${item.active ? "active" : ""}`}>
              ...
              {item.active && (
                <div
                  className="active-indicator"
                  style={{ width: widths[item.label] ? `${widths[item.label]}px` : undefined }}
                />
              )}
            </div>
          ))}
